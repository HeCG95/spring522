...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			/**
			 * 得到内部维护的beanFactory，为什么需要返回一个bean工厂？因为要对工厂进行初始化
			 * 如何得到？
			 * 因为AnnotationConfigApplicationContext继承了GenericApplicationContext，
			 * 在初始化AnnotationConfigApplicationContext时执行了GenericApplicationContext的构造方法初始化了beanFactory
			 * 而obtainFreshBeanFactory()最终执行的是GenericApplicationContext中的getBeanFactory()方法将beanFactory返回
			 */
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//能够在类中获得到各种*Aware（*Aware都有其作用）
		// 内部维护的是一个list，在bean实例化时会从list中取出插手bean实例化的过程
